...
//Parameter1 : 8-digit number
//brief: Create a letter related to the input number
//return: 1 letter string.
//Catch: String: "Invalid parameter"
...
function dniLetter( dni ) {
	...
	if(dni < 0 || isNaN(dni) ){ //Condition for Exercise 2

		result = "Invalid parameter";

	}else{

		result = lockup[(dni%23)]; // store the char from the position of the lockup string.
	}
...
//Parameter 1: Array (String)
//brief: Calculate the word with most characters in the array. It returns the amount of characters that word has.
//Return: Number
...
array_lengths.push( findLongestWord( ["Red", "Blue", "Green", "Purple", "Teal"] ) );
array_lengths.push( findLongestWord( ["Lion", "Dog", "Elephant", "Chicken"] ) );
array_lengths.push( findLongestWord( ["Chip & Fish", "Potatoes", "Pizza"] ) );
...
// Commentaries above is due a malfunction after adding the second part of the Exercise


//Parameter1 : Array (Numbers)
//Brief: Calculate the ROUNDED! average of the numbers inside an array.
//Return: Number (Rounded)
function calculateAverage( array )
{
	var average = 0;
	for(i = 0; i<array.length; i++){
		average = average + parseInt(array[i]);
	}
...
// Parameter1 : String
// Brief: Creates an Object whose properties are the different characters contained on a string. The value of those
//		  properties are the amount of times those characters repeat on that string.
// Return: Object 
function charFreq( string ) {
	...
	for(i=0; i<string.length; i++){
		if(freqCounter[string[i]] === undefined){
			...
			freqCounter[string[i]] = freqCounter[string[i]] +1;
		}
	}
...
//Parameter1: Array
//Brief: Call several functions to modify an array and make it awesome. (See documentation of every function called)
//Return Array
...
//Parameter1: Array
//Brief: It deletes every string equal to "*" in an array.
//Return: Array
...
//Parameter1: Array
//Brief: Lowercase every string EQUAL to a voyel on an array and Uppercase any string EQUAL to a consonant on an array.
//Retrun: Array
...
//Paramaeter1: Number
//Brief: Sums all the digits of a number until there is only one digit.
...
function sumDigits(number){
	...
	do{
		for(x=0;x<digits.length;x++){
			sumaTotal = sumaTotal + parseInt(digits[x])
		}
		//console.log(sumaTotal);
		digits = (""+sumaTotal).split("");
		sumaTotal=0;
	}
...
//Parameters1: Array
//Brief: Call the function sumDIgits for every NUMBER inside an array. Ignores the rest of the items.
//Return: Array
function beautifyNumbers( array )
{
	...
	for(i=0;i<array.length;i++){
					...
					console.log(i + " Ha pasado por aquí");	

		if(!isNaN(array[i])){
			if(array[i].toString().length>1){		
							
				 array[i] = sumDigits(array[i]);
			}
		}
	}
return array;
...
//Parameter1: Array
//Brief: Convert all the items of an array into a single string.
//Return: String
